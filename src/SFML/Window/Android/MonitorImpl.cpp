...
void fillDisplayMetrics(JNIEnv& env, jobject display, int sdkInt, MonitorImpl& impl)
{
    ...
    {
        if (auto* const name = static_cast<jstring>(env.CallObjectMethod(display, methodGetName)))
        {
    ...
    if (const jmethodID methodGetRefreshRate = env.GetMethodID(classDisplay, "getRefreshRate", "()F"))
    {
        ...
        impl.refreshRate         = (refreshRate > 0.f) ? static_cast<unsigned int>(std::lround(refreshRate)) : 60;
    }
